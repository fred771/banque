...
public class Banque implements Serializable{  
	/**
	* Serializable permet les sauvegardes
	* on sauvegarde un objet Banque, c'est JAVA qui gére seul
	* Ne rien modifier sinon on perdra toutes les données 
  	*/
	...
	Compte getCompte(int numComp){
		...
		for ( i=0; i<lesComptes.size();i++){
		    if (lesComptes.get(i).getNumComp()==numComp) 
		    	break;
		}
	...
	try{ 
	    ...
	    return  new Compte(id,sold,this);
	}
	catch(Exception e){
